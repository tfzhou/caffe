...
void SigmoidFocalLossLayer<Dtype>::Forward_cpu(
  ...
  for (int i = 0; i < bottom[0]->count(); ++i) {
    ...
    // scale
    Dtype scale = (target[i] == 1 ? alpha_ : 1 - alpha_) * pow(1 - (target[i] == 1 ? sigmoid_output_data[i] : 1-sigmoid_output_data[i]), gamma_);
    Dtype orgLoss = input_data[i] * (target[i] - (input_data[i] >= 0)) -
...
void SigmoidFocalLossLayer<Dtype>::Backward_cpu(
  ...
  if (propagate_down[0]) {
    ...
    for(int i = 0; i < count; i++) {
        ...
        Dtype item1 = alpha_ * pow(1 - (target[i] == 1? sigmoid_output_data[i] : 1 - sigmoid_output_data[i]), gamma_);
        Dtype item2 = gamma_ * sigmoid_output_data[i] * (input_data[i] >= 0 ? -log(1+exp(-input_data[i])) : input_data[i] - log(1+exp(input_data[i])));
        Dtype item3 = sigmoid_output_data[i] - 1;

        bottom_diff[i] = item1 * (item2 + item3);
    }
